...
from app.services.experiment_engine import run_experiment, BUILTIN_TEMPLATES, OUTPUT_ROOT
        ...
        try:
            ...
            if run.project_id and res['charts']:
                ...
                if proj:
                    ...
                    # [fix-final-B] 把产物路径也回写到关联的 QuestionTask.result
                    try:
                        from app.database.models import QuestionTask as _QT, ScienceQuestion as _SQ
                        _task = None
                        if run.question_task_id:
                            _task = (await db.execute(
                                select(_QT).where(_QT.id == run.question_task_id)
                            )).scalar_one_or_none()
                        if _task is None and proj and proj.id:
                            _sq = (await db.execute(
                                select(_SQ).where(_SQ.question_id == proj.id)
                            )).scalar_one_or_none()
                            if _sq:
                                _task = (await db.execute(
                        ...
                        if _task is not None:
                            ...
                            _figs = list(_r.get("figures") or [])
                            _vids = list(_r.get("videos") or [])
                            for c in res.get("charts", []):
                                p = c.get("path") if isinstance(c, dict) else str(c)
                                if p and p not in _figs:
                                    _figs.append(p)
                            if res.get("video_path") and res.get("video_path") not in _vids:
                                _vids.append(res.get("video_path"))
                            _r["figures"] = _figs
                            _r["videos"] = _vids
                            _task.result = _r
                            await db.commit()
                            print(f"[lab] run {run.id} -> task {_task.id} 回写 figures={len(_figs)} videos={len(_vids)}")
                    except Exception as _e:
                        print(f"[lab] task result 回写失败(不阻塞): {_e}")
                    await db.commit()
...
async def video_file(run_id: int, token: str = Query(None),
                     db: AsyncSession = Depends(get_db),
                     user: User = Depends(get_current_user_flexible)):
    run = (await db.execute(select(ExperimentRun).where(
        ExperimentRun.id == run_id, ExperimentRun.user_id == user.id))).scalar_one_or_none()
...
async def batch_delete_runs(req: BatchDeleteRequest,
                            db: AsyncSession = Depends(get_db),
                            user: User = Depends(get_current_user)):
    """批量删除实验运行记录及其输出文件"""
    import shutil
    ...
    for rid in req.run_ids:
        try:
            run = (await db.execute(select(ExperimentRun).where(
                ExperimentRun.id == rid, ExperimentRun.user_id == user.id))).scalar_one_or_none()
            if not run:
                errors.append({'run_id': rid, 'error': '记录不存在或无权限'})
                continue
            # 删除输出目录
            out_dir = os.path.join(OUTPUT_ROOT, str(rid))
            if os.path.isdir(out_dir):
                shutil.rmtree(out_dir, ignore_errors=True)
            await db.delete(run)
            deleted += 1
        except Exception as e:
            errors.append({'run_id': rid, 'error': str(e)[:200]})
    await db.commit()
    return {'message': f'已删除 {deleted} 条记录', 'deleted': deleted, 'errors': errors}


@router.delete('/runs/{run_id}')
async def delete_single_run(run_id: int,
                            db: AsyncSession = Depends(get_db),
                            user: User = Depends(get_current_user)):
    """删除单条实验运行记录"""
    import shutil
    ...
    await db.delete(run)
    await db.commit()
    return {'message': '已删除', 'run_id': run_id}
