...
BUILTIN_TEMPLATES = [
    ...
    },
    {
        ...
        "    # 拆分 2x2 子图为独立图表\n"
        "    for _ci, (_cax, _cn) in enumerate(zip(axes.flat, sample_sizes)):\n"
        "        _cfig = plt.figure(figsize=(6, 4))\n"
        "        _cmeans = [np.mean(np.random.exponential(1, _cn)) for _ in range(n_experiments)]\n"
        "        plt.hist(_cmeans, bins=50, density=True, alpha=0.7, color='steelblue', edgecolor='white')\n"
        "        _cmu, _csigma = np.mean(_cmeans), np.std(_cmeans)\n"
        "        _cx = np.linspace(_cmu - 4*_csigma, _cmu + 4*_csigma, 200)\n"
        "        plt.plot(_cx, stats.norm.pdf(_cx, _cmu, _csigma), 'r-', lw=2)\n"
        "        plt.title(f'Sample n={_cn}')\n"
        "        plt.xlabel('Sample Mean')\n"
        "        plt.ylabel('Density')\n"
        "        plt.tight_layout()\n"
        "        plt.savefig(f'clt_n{_cn}.png', dpi=150, bbox_inches='tight')\n"
        "        plt.show()\n"
        "        plt.close(_cfig)\n"
            "plt.show()\n"
            "\n"
