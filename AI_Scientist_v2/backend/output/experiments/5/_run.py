...
OUT = r'D:\111-1\AI_Scientist_v2\backend\output\experiments\5'
CDIR = r'D:\111-1\AI_Scientist_v2\backend\output\experiments\5\charts'
_CHARTS = []
...
sns.scatterplot(x='time', y='dark_matter_content', data=df, hue='model_choice')
plt.title('Dark Matter Content vs Time')
plt.xlabel('Time')
plt.ylabel('Dark Matter Content')
plt.legend(title='Model Choice')
plt.show()

# 回归线
plt.figure(figsize=(10, 6))
sns.regplot(x='time', y='dark_matter_content', data=df, scatter_kws={'alpha':0.5})
plt.title('Regression Line: Dark Matter Content vs Time')
plt.xlabel('Time')
plt.ylabel('Dark Matter Content')
plt.show()
